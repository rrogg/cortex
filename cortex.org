...
=my-format-sexp= will replace the symbolic expressions within =sexp= with
...
The =my-modus-theme-after-load-hook= can be used to run additional code
specifically after a theme has been loaded.

With the help of =my-format-sexp= each of the two theme variants get a separate
function (=my-modus-operandi= and =my-modus-vivendi=) from a single source of
code. All customizations can be done here.

#+BEGIN_SRC emacs-lisp
(use-package emacs
  :config
  (defvar my-modus-theme-after-load-hook nil
  ...
  (dolist (themes '("operandi" "vivendi"))
    (my-format-sexp
      (defun my-modus-%1$s ()
        ...
        (run-hooks 'my-modus-theme-after-load-hook))
      themes)))
#+END_SRC
...
(use-package emacs
  ...
  (defun my-themes-toggle ()
    "Toggle between light theme (`my-modus-operandi') and dark theme (`my-modus-vivendi')."
    (interactive)
    (if (eq (car custom-enabled-themes) 'modus-operandi)
      ...
      (my-modus-operandi)))
  :hook (after-init-hook . my-modus-operandi)
  :bind ("<f5>" . my-themes-toggle))
#+END_SRC
